...
if adjusted_larsson
    ...
    IRF    = ( repmat((a - theta - (Ki./Vb) ),1,num_time_stamps) .* exp(-repmat(a,1,num_time_stamps).*repmat(time_vec,Num_iterations,1)) - ...
        repmat((b - theta - (Ki./Vb) ),1,num_time_stamps) .* exp(-repmat(b,1,num_time_stamps).*repmat(time_vec,Num_iterations,1)) ) ./ (repmat(a - b,1,num_time_stamps)); % No units
end
